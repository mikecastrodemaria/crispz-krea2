...
- **Ollama (optional)**: **Describe** (image→prompt, in a **style** you pick), **Improve
  prompt**, and **Vision Mix** (blend several reference images into one prompt). Models unload from VRAM after
...
- **Describe** (Input Image → Describe): turns an image into a prompt with an Ollama
  ...
  **Length** goes from 60 to 300 words, and the accordion under it shows the exact
  ...
  muse-glimmer, each description regenerated by crispz-klein at the same seed): naming the medium
  first keeps a drawing a drawing, and a quoted text comes back clean only when it is
  given in one piece; asking for the era (measured 2026-09-12) raised the portrait's
  fidelity from 0.54 to 0.65 on both models. The other styles follow the same rules. A small vision model
  (Agents-A1-4B, 3.3 GB) describes in 1-5 s next to the image model; a 28B one
  (muse-glimmer, ~21 GB) is richer but needs the GPU to itself.
...
- **Where**: UI **Generate** (txt2img, img2img, Reference (Omni)), the **Inpaint / Outpaint** tab, the **job queue** and the **X/Y/Z grid** (the *Prompt S/R* axis works on the raw text, groups included), the **CLI** (`--prompt`, `--negative`, `--xyz` per cell, `--serve`) and the **`czp` protocol** (img2img, Omni and Inpaint stay hidden on Krea 2; their handlers are wired the same way). A style `{prompt}` placeholder is never taken for a group (a group needs a `|` or a `N$$` prefix).
...
- **Describe the result** in the prompt (the full outfit/scene). **Auto-describe** runs
  automatically when the prompt is empty (the caption model: local BLIP, or an Ollama
  vision model) to keep the fill coherent with the center — or check it to also prepend a description to your prompt.
...
Krea 2 has no edit pipeline, so the **Reference (Omni)** tab never shows here: dressing
a person with a garment from a product shot, or changing a head, is done in
crispz-klein ([Try-on and casting](https://github.com/mikecastrodemaria/crispz-klein#try-on-and-casting-reference-omni--face-swap)). The face swap works here on its own,
on txt2img renders: turn the face **detailer off**, turn **Apply face swap to result**
on, and the face of the source image replaces every face of the render.
...
|---|---|
| Krea 2 Turbo / Raw | a Qwen3-VL-4B: `qwen3_vl`, text hidden size 2560, 36 layers |

A text-only Qwen3 is refused (another model type), so the Qwen3-4B abliterations made for
FLUX.2 Klein / Z-Image do **not** fit Krea 2. The layer count matters too: Krea 2 reads
fixed hidden layers (2, 5, …, 35), and a shallower or deeper encoder would feed it the
wrong ones. Env `KREA2_TEXT_ENCODER`.
