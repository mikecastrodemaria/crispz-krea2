"""crispz-krea2 - CLI (mode batch / scripting) + serveur HTTP persistant (FastAPI).
...
def cli_main(argv=None):
    ...
    parser = argparse.ArgumentParser(
        description="crispz-krea2 CLI (txt2img + upscale). No args: launches the Gradio UI.",
    ...
    parser.add_argument("--reframe", metavar="W:H",
                        help="Reframe -i to a target aspect ratio (e.g. 16:9 / 9:16), then exit. "
                             "NB Krea 2: only --reframe-fit cover works (no inpaint pipeline); "
                             "contain raises the clear UnsupportedFeature message.")
    parser.add_argument("--reframe-fit", choices=["contain", "cover"], default="contain",
                        help="--reframe mode: contain = keep the whole image and OUTPAINT the "
                             "borders (UNAVAILABLE on Krea 2); cover = fill the ratio then "
                             "centre-crop (no generation, works here).")
    parser.add_argument("--expand", metavar="SIDES",
                        help="Directional outpaint of -i (left,right,top,bottom or 'all'). "
                             "UNAVAILABLE on Krea 2 (no inpaint pipeline): raises the clear "
    ...
    parser.add_argument("--inpaint-mask", metavar="PATH",
                        help="Inpaint the WHITE area of this mask. UNAVAILABLE on Krea 2 "
                             "(no inpaint pipeline): raises the clear UnsupportedFeature message.")
    parser.add_argument("--inpaint-denoise", type=float, default=1.0, metavar="1.0",
                        help="Inpaint strength for --inpaint-mask.")
    parser.add_argument("--force-ratio", metavar="W:H",
                        help="Force an aspect ratio on the Upscale INPUT before processing "
                             "(crop mode works on Krea 2). Also via config "
                             "'force_upscale_ratio' or env CZ_FORCE_RATIO.")
    parser.add_argument("--force-ratio-mode", choices=["crop", "extend"], default=None,
                        help="crop = centre-crop (works on Krea 2, default); extend = outpaint "
                             "the missing bands (UNAVAILABLE on Krea 2: no inpaint pipeline).")
    ...
    # Ratio force sur l'entree Upscale (comme le radio UI). Le mode peut aussi venir de
    # la config; le flag explicite gagne. NB: 'extend' leve UnsupportedFeature sur Krea 2.
    if args.force_ratio:
    ...
    # --reframe W:H : reframe -i (cover = crop; contain -> UnsupportedFeature sur Krea 2)
    if args.reframe:
        if not args.input or not os.path.isfile(args.input):
    ...
    # --expand / --inpaint-mask : passent par outpaint_directions / inpaint_run, qui
    # levent le message UnsupportedFeature clair sur Krea 2 (pas de pipeline inpaint).
    if args.expand:
        if not args.input or not os.path.isfile(args.input):
