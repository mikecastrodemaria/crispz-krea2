...
FAMILY_CAPS = {"img2img": False, "inpaint": False}
...
OMNI_SUPPORTED = False   # no instruction-edit/omni pipeline for this model family
                         # (_load_omni raises) - config can never turn refs on here
...
def _expand_spec(spec, seed):
    ...
    from cz_prompt import expand_prompt_pair
    out = dict(spec)
    ...
    # No <lora:...> in the negative: a syntax fragment never reaches the encoder.
    neg = negative or ""
    clean = _LORA_TAG.sub(" ", neg)
    out["negative"] = re.sub(r"\s{2,}", " ", clean).strip() if clean != neg else neg
    out["loras"] = loras
    return out
