...
def _ui_set_text_encoder(src):
    ...
    return (f"Text encoder: **{cz_pipeline._encoder_label(src)}**. Tokenizer, VAE and "
            f"transformer stay the base repo's." + reload)
...
def _ui_edit(mode, editor_value, dirs, ratio, fit, auto_describe, harmonize, harmonize_denoise,
    ...
    try:
        ...
        # {a|b|c} + wildcards developpees (liees a cette seed).
...
def _ui_generate(prompt, negative, styles, style_random, use_input, input_image,
    ...
    try:
        ...
        def _prep(s, i):
            ...
            wildcards du positif et du negatif, PUIS styles. Renvoie (prompt, negatif,
            styles choisis)."""
        ...
        if use_input and input_mode == "Reference (Omni)" and omni_ready:
            ...
            for i in range(n):
                ...
                full_prompt, full_negative, picked_styles = _prep(s, i)
                if style_random:
...
def _api_cli_edit(spec_json):
    """Endpoint api_name='cli_edit': image + instruction -> image via le
    pipeline omni/edit (protocole CLI v1, op edit)."""
    import cz_protocol
    return json.dumps(cz_protocol.handle_edit_json(spec_json),
                      ensure_ascii=False)
...
def build_ui():
    ...
    with gr.Blocks(title=f"crispz-krea2 {APP_VERSION}", theme=gr.themes.Default(), css=FOOOCUS_CSS,
        ...
        cle_in = gr.Textbox(visible=False)
            ...
            with gr.Column(scale=3):
                ...
                with gr.Row():
                    ...
                    auto_upscale_cb = gr.Checkbox(
                        ...
                        scale=4,
                        label="Upscale after generate — chain each txt2img or Reference (Omni) image through the "
                ...
                detail_hands_cb = gr.Checkbox(
                    value=cz_detailer.HAND_ENABLED and HAS_IMG2IMG, visible=HAS_IMG2IMG,
                            ...
                            with gr.Row():
                                ...
                                with gr.Column():
                                    ...
                                    refine_first_cb = gr.Checkbox(value=bool(CONFIG.get("default_refine_first", False)) and HAS_IMG2IMG,
                                                                  ...
                                                                  visible=HAS_IMG2IMG)
                                    # Vary = img2img pur -> gate comme le reste (HAS_IMG2IMG)
                                    with gr.Row(visible=HAS_IMG2IMG):
                        ...
                        with gr.Accordion("\U0001F4E6 Checkpoints (switch model)", open=True):
                            ...
                            with gr.Column():
                                te_dd = gr.Dropdown(
                                    ...
                                    info="Default: the base repo's own Qwen3-VL. Or a transformers "
                                         "folder (config.json + weights) or HF repo of the SAME "
                                         "size and depth, e.g. an abliterated Qwen3-VL-4B. Pick or "
                                         "paste a path; changing it reloads the model and clears "
                                         "the prompt cache.")
                                with gr.Row():
