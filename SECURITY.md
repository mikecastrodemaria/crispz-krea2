...
crispz-krea2 is developed on `main` with no maintenance branches: only the **latest
commit on `main`** receives fixes. The current version is in `cz_core.py`
(`APP_VERSION`) and shown in the browser tab title.

## Scope

crispz-krea2 is a **local desktop application**. `build_ui().launch()`
...
rather than vulnerabilities in crispz-krea2.
...
that distinction meaningful: crispz-krea2 is a local app bound to 127.0.0.1, and the
