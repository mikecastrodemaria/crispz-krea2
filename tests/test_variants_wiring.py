"""{a|b|c} variant groups wired into crispz-krea2 (prompt_variants + cz_prompt).
...
Covers the family rules for the variant syntax:
  ...
  - protocol: <lora:...> tags are read on the EXPANDED text (a LoRA in an
    unpicked option is never applied);
...
class _FakePipeline(types.ModuleType):
    ...
    def __init__(self):
        ...
        self.calls = {"loras": None, "gen": None}
...
def _ui_txt2img(prompt, negative, n, seed):
    ...
    # 4th parameter named gen_steps: txt2img_run is also called with steps=.
    ...
    try:
        cz_ui._ui_generate(
            ...
            64, 64, 4, n, seed, 0.0, cz_pipeline.OFFLOAD_MODE,
...
def test_ui_generate_resolves_seed_minus_one():
    import cz_pipeline
    with _wildcards({}):
        calls = _ui_txt2img("a {red|blue|green|black} car", "", 1, -1)
    (fp, s, _fn), = calls
    assert s >= 0 and cz_pipeline._LAST_SEED == s
    assert fp == P._apply_wildcards("a {red|blue|green|black} car", P._seed_rng(s),
                                    index=0)
