...
every such path, and the protocol reads `<lora:...>` tags on the expanded text, so a
LoRA written in an option that was not picked is never applied. `czp upscale` now
returns `seed_used`. The hidden img2img, Omni and Inpaint handlers are wired the same
way.
...
do not touch now stay as they are. Their final check, and step [4/5] of `boot_check.bat`, imported the Z-Image
pipelines, a leftover from crispz-studio that passed without proving anything:
they now import the Krea 2 ones (Krea2Pipeline). `update.bat` now ends with an explicit exit
...
A **LoKr** disappeared outright: merged into the weights by the support added earlier
today, it sits in no PEFT adapter at all, so nothing recorded it. `_APPLIED_LOKRS` is
folded into the same list.
...
NB crispz-krea2: both detailers stay hidden (HAS_IMG2IMG gate) - diffusers
has no Krea2Img2ImgPipeline yet, and the detail pass is an img2img refine.
The port keeps the family aligned for the day the pipeline lands.
...
different base (a *Z-Image* or *FLUX* version sitting on top of the list of a *Krea 2* LoRA), and every local copy was then flagged as
...
considers versions published for that base — normalised comparison, so *Krea 2* / *krea2*
